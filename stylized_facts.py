...
def test_leverage_effect(returns: np.ndarray,
    ...
    Metric: Pearson correlation between r_t and |r_{t+1}|.
    A negative correlation indicates that negative returns raise future volatility more.
    Pass: correlation < 0 (sign test; robust across all model types).
    """
    flat = _to_1d(returns)
    r_t = flat[:-1]
    ...
    if real_returns is not None:
        flat_real = _to_1d(real_returns)
        r_t_r = flat_real[:-1]
        abs_r_next_r = np.abs(flat_real[1:])
        corr_real = float(np.corrcoef(r_t_r, abs_r_next_r)[0, 1])
